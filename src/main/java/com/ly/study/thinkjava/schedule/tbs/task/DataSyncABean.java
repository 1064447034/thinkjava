...
public class DataSyncABean implements IScheduleTaskDealSingle<OrderInfo> {
	...
	@Override
	public List<OrderInfo> selectTasks(String taskParameter, String ownSign, int taskItemNum,
			List<TaskItemDefine> queryCondition, int eachFetchDataNum) throws Exception {
		...
		System.out.println("dataSyncABean");
		/* 场景A：将tbOrder表中的数据分8个任务项，每次取200条数据， 同步到tbOrder_copy表中。 */
/*		String sql = "select * from tbOrder " + "where " + " BillNumber not in (select BillNumber from tbOrder_copy) "
				+ " and RIGHT(BuildDate,1) in (" + condition + ") " + "limit " + eachFetchDataNum;
		...
		MySQLHelper.free(rs, rs.getStatement(), rs.getStatement().getConnection());
*/
		return result;
	}
...
/*		String sql = "insert into tbOrder_copy values('" + task.BillNumber + "','" + task.BuildDate + "','"
				+ task.Customer + "','" + task.GoodsName + "'," + task.Amount + "," + task.SaleMoney + ")";
